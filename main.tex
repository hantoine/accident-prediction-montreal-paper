...
day~\cite{Peden2004}. In the last few years, the number of road traffic deaths continued to climb, reaching 1.35 million in 2016\cite{road_safety_report}. More particularly in Quebec, Canada, hundreds of people were killed in 2018, more than a thousand were seriously injured and tens of thousands of small injuries were noticed\cite{saaq}.
...
\AH{I've seen the term ``open data movement" used more often, but it's true it's not really a movement. Which one should we use ?}
...
we created a very basic model using only the count of accident of the road segment.
We used the results of this model as a baseline. The probability of accident given 
by this model for an example whose road segment has a count of accident of $n$, is the percentage
of the examples with a count of accident higher than $n$ which are positive.
...
\AH{should we talk about the performance difference for a threshold of 0.5 ?}
