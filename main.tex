...
big data analytics.
Such models could be used in the context of road accident prevention, \english{but also}
...
\BJ{???: mais pas a Montreal (comme indiqu\'e dans le titre), peut-etre plusieurs ann\'ees, ou sinon le cout pour SAAQ}
\TG{Is the PR in?} 
\BJ{Not sure the details of scala/Python are needed in the abstract}.
\BJ{keywords in conference paper? check the template}
...
day~\cite{Peden2004}. More particularly in Quebec, Canada, hundreds of people were killed in 2018, more than a thousand were seriously injured and tens of thousands of small injuries were noticed\cite{saaq}.

On the other hand, Big Data Analytics has emerged in the last decade as a set of techniques allowing data scientists to extract meaningful information from large amounts of complex and heterogeneous data \cite{Gandomi2015}. In the context of accident prediction, such techniques could provide insights on the conditions leading to an increased risk of road accidents, which in return, could be used to develop traffic-related policies and prevention operations. 

\BJ{I agree with Tristan: Introduction = one section for motivation/general background + one section for your project description and very high level related work + your contributions + plan of the paper}
...
Governments, states, province and municipalities collect and manage data for their internal operations. In the last decade, the open data trend has emerged, encouraging governments to make the data they collect available to the public as ``open data".
...
This trend is made possible by the progress of information technology which allows the sharing of large amounts of data easily. In 2009, Canada, USA, UK and New Zealand, announced new initiative towards opening up public information. It is in this spirit that the Government of Canada launched its first-generation of the Open Data Portal in 2011~\cite{opendata101}, giving access to several public datasets. Since 2012, the city of Montreal is also taking part in the open data trend.

\BJ{\sout{movement} trend}
...
An article\cite{mediumarticle} from the website ``Medium" presents a study of road accident prediction in the state of Utah with apparent good performances. This article has inspired us to try using public datasets to build a machine-learning model for road vehicle collision prediction. We used datasets 
...
In this study, we assembled a dataset containing road vehicle collisions, a dataset describing the Canadian road network, and a dataset containing historical weather information. Using these datasets, we created positive examples, corresponding to the occurrence of a collision, and negative examples, corresponding to the non-occurrence of a collision. For each example, we extracted from the datasets relevant features for accident prediction. Then, we built several predictions model using these example using various machine learning algorithms. We focused on tree-based machine-learning algorithms because they have proven their efficiency compared to classical statistical methods \cite{Chang2005, Chang2005b} and they allow for easier interpretation than deep learning algorithms. We first used the Random Forest algorithm. We then implemented in Apache Spark and used the Balanced Random Forest algorithm\cite{Chen2004}, a variation of Random Forest aimed at better dealing with data imbalance. Finally, we  tried the XGBoost algorithm, a gradient tree boosting algorithm which has been used successfully for many machine learning problems and can handle data imbalance\cite{xgboost_doc}.
...
Random Forest algorithm with further under-sampling (RF), with the Balanced Random Forest algorithm (BRF), and with the XGBoost algorithm (XGB) on the test set. The values of the hyper-parameter we used and more details about the results are available on the Github repository of the project\footnote{\url{https://github.com/big-data-lab-team/accident-prediction-montreal/tree/master/results}}
...
As stated previously, the measure of the accuracy is not a good metric for road accidents
...
positive rate. For these reasons, we decided not to use the measure of the accuracy. Instead
...
city of Montreal using open data provided by Montreal and the Government of Canada. From three different datasets, we have built road vehicle collision
prediction models using tree-based algorithms. Our best model can predict $85\%$
of road accidents in the area of Montreal with a false positive rate of $13\%$. Our models predict the occurrence of a collision at high space resolution and hourly precision. In practice, this
means our models can be used to identify the most dangerous road
segments every hour, in order to take actions to reduce the risk of accident. Moreover, we believe that our work
